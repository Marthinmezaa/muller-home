...
- Spec de panel de Super Admin aprobada (openspec/changes/panel-super-admin/spec.md) y **módulo implementado**: `/panel/pagos` (lista de compras con comprador/paquete/estado, ver comprobante vía URL prefirmada, aprobar/rechazar solo en `PENDING`) y `/panel/catalogo` (paquetes activos + formulario de alta). Ambos links del nav solo aparecen si `role === 'SUPER_ADMIN'`.
- Gap de backend corregido: `findAllPurchases()` ahora incluye `package` y `buyer` (`fullName`, `email`) — sin eso la pantalla no tenía qué mostrar más allá de ids.
- Gap detectado y dejado fuera de alcance a propósito: **sigue sin existir ninguna pantalla frontend para comprar un paquete** (ni para asesor ni para franquicia) — la API lo soporta desde siempre, nadie construyó esa UI. Spec propia cuando toque.
- Diagrama de arquitectura (`docs/architecture/system-design.html`) actualizado con las rutas nuevas del panel — se mantiene al día en cada cambio de arquitectura relevante, sin que haga falta pedirlo cada vez.
- Spec de compra de paquete aprobada (openspec/changes/comprar-paquete/spec.md) y **módulo implementado**: `/panel/comprar` (`apps/web/src/app/panel/comprar`), catálogo de paquetes activos con formulario de compra (elegir paquete, subir comprobante) y sección "Mis compras" con estado y reenvío de comprobante si `REJECTED`. Link de nav visible para `franchise_admin` o `advisor` sin `franchiseId` (un asesor ya invitado a una franquicia no compra paquete propio, `assertCanBuy` ya lo bloqueaba en la API).
- Gap de backend corregido: `findMyPurchases` no incluía `package` — sin eso la pantalla no tenía nombre/tipo de paquete para mostrar.
- Paleta de marca aplicada a fondo en `apps/web`: tokens Tailwind v4 (`@theme` en `globals.css` — `--color-brand-navy`, `--color-brand-gold`, `--color-brand-ink`, `--color-brand-cloud`) reemplazando los ~18 hex sueltos (`bg-[#3B3A72]`, etc.) que había en botones y nav. Fondo/texto claro del `body` pasan a usar `brand-cloud`/`brand-ink` en vez de blanco/gris puro.
- Logo aplicado por primera vez en el frontend: `SiteHeader.tsx` (isotipo del búho + "Muller Home"), montado en el root `layout.tsx` — aparece en toda la app (portal público y panel, este último conserva su propio nav debajo). Favicon reemplazado por el isotipo (`apps/web/src/app/icon.png`, convención de Next.js — se borró el `favicon.ico` default). Assets recortados del logo original (`docs/brand/logo.jpeg`) con `sharp` en `apps/web/public/brand/owl-mark.png` y `src/app/icon.png` — mismo isotipo, sin el wordmark "Producciones" para no mezclarlo con la marca "Muller Home".
- Cupo disponible agregado (`GET /packages/quota`, ya existía en la API) ahora se muestra en `/panel` (home del panel) para `advisor`/`franchise_admin` — con link directo a `/panel/comprar` cuando llega a 0. No se muestra para `super_admin` (no compra paquetes, siempre daría 0). Con esto **el alcance de Fase 1 (MVP) queda completo** — todos los módulos de `openspec/changes/` están implementados.
- Rediseño visual "mejorar craft" aplicado en todo `apps/web` (decisión confirmada con Marthin: se mantiene la marca navy/oro/búho, no se toca Fase 2/identidad definitiva — es tipografía, espaciado y diseño de componentes).
  - Tipografía: `body` pasa a usar Geist (`var(--font-sans)`, ya cargada por `next/font` desde el arranque del proyecto pero nunca conectada — hardcodeaba Arial) en vez del fallback genérico.
  - `apps/web/src/lib/ui.ts` (nuevo): clases compartidas de botón/card/input/badge como strings, no componentes — se aplican igual a `<button>`, `<Link>` o `<div>` sin necesitar una prop `as`. Reemplaza los estilos ad-hoc repetidos en paneles, forms y cards de todo el frontend.
  - `PURCHASE_STATUS_STYLES` centralizado en `lib/types.ts` (estaba duplicado en `/panel/pagos` y `/panel/comprar`).
  - Accesibilidad: los forms (`SearchFilters`, `PropertyForm`, `LeadForm`, `AcceptInviteForm`, login, invitar asesor) que solo tenían `placeholder` ahora tienen `<label>` real (`sr-only`) — ya se había tocado cada input de todas formas para el cambio de clase.
  - `SiteHeader` pasa a sticky con blur; por eso se revirtió parcialmente la simplificación de `globals.css` del PR #37 — `--color-background`/`--color-foreground` en `@theme inline` ahora sí tienen un consumidor real (`bg-background/80`).
- Próximo paso: identidad visual definitiva sigue siendo Fase 2 (post-validación con cliente) — este rediseño no la adelanta, solo mejora la ejecución dentro de la marca provisoria actual.
