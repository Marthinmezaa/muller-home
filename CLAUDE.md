...
- Spec de panel de Super Admin aprobada (openspec/changes/panel-super-admin/spec.md), **implementación pendiente**: `/panel/pagos` (aprobar/rechazar compras de paquete, ver comprobante) y `/panel/catalogo` (ver paquetes activos, crear uno nuevo) — la lógica ya existía en `packages.service.ts` desde su spec original (PR #9), esto es el frontend que le faltaba.
- Gap de backend detectado al escribir esa spec (a resolver junto con la implementación): `findAllPurchases()` no incluye las relaciones `package`/`buyer`, sin eso la pantalla no tiene qué mostrar más allá de ids.
- Gap más grande detectado, dejado fuera de alcance a propósito: **no existe ninguna pantalla frontend para comprar un paquete** (ni para asesor ni para franquicia) — la API lo soporta desde siempre, nadie construyó esa UI. Spec propia cuando toque.
- Próximo paso: implementar el panel de Super Admin (código de este spec), después aplicar la paleta de marca más a fondo o encarar el flujo de compra del lado del asesor — a definir con Marthin.
