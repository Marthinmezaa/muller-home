...
- Próximo paso: panel de Asesor (propiedades propias, leads, métricas básicas) en `apps/web` — hoy `GET /leads` y `GET /properties/mine` existen en la API pero no hay UI en el frontend, o aplicar la paleta de marca más a fondo — a definir con Marthin.
