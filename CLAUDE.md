...
- Spec de panel de franquicia aprobada (openspec/changes/panel-franquicia/spec.md), **implementación pendiente**: `/panel/equipo` (ver asesores del equipo, invitar por link) y `/panel/bajas` (aprobar/rechazar solicitudes de baja de propiedad), más `/aceptar-invitacion` (frontend que faltaba para que la invitación funcione de punta a punta).
- Gaps de backend detectados al escribir esa spec (a resolver junto con la implementación): faltan `GET /auth/franchise/members` y `GET /properties/deletion-requests` — sin ellos no hay forma de listar equipo ni solicitudes pendientes, aunque las acciones (invitar, aprobar, rechazar) ya existían en la API.
- Próximo paso: implementar el panel de franquicia (código de este spec), aplicar la paleta de marca más a fondo, o el panel de Super Admin (aprobar pagos, gestión de paquetes) — a definir con Marthin.
